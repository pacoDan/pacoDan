...
```sh
sudo pacman -S --noconfirm zsh gcc make cmake unzip neofetch screenfetch gettext gdb tree git curl wget xclip xsel wl-clipboard ripgrep fd lazygit htop openssh docker plank flameshot gpaste tilix nemo dolphin docker-compose timeshift reshift
```
```sh
